<?php
/**
 * API Endpoint: Obtener Plantilla por Defecto de WhatsApp del Usuario
 * 
 * Endpoint para obtener la plantilla de WhatsApp por defecto del usuario.
 * 
 * Método: GET
...
try {
    ...
    error_log('Error PDO en get-default-whatsapp-template.php: ' . $e->getMessage());
    error_log('SQL State: ' . $e->getCode());
    sendJsonResponse(false, null, 'Error de base de datos: ' . $e->getMessage(), 500);
} catch (Exception $e) {
    error_log('Error en get-default-whatsapp-template.php: ' . $e->getMessage());
    error_log('File: ' . $e->getFile() . ' Line: ' . $e->getLine());
    sendJsonResponse(false, null, 'Error interno del servidor: ' . $e->getMessage(), 500);
} catch (Error $e) {
    error_log('Fatal Error en get-default-whatsapp-template.php: ' . $e->getMessage());
    error_log('File: ' . $e->getFile() . ' Line: ' . $e->getLine());
