<?php
/**
 * API Endpoint: Guardar Plantilla por Defecto del Usuario
 * 
 * Endpoint para guardar la plantilla de email por defecto del usuario.
 * 
 * Método: POST
...
try {
    ...
    error_log('Error PDO en save-default-template.php: ' . $e->getMessage());
    error_log('SQL State: ' . $e->getCode());
    sendJsonResponse(false, null, 'Error de base de datos: ' . $e->getMessage(), 500);
} catch (Exception $e) {
    error_log('Error en save-default-template.php: ' . $e->getMessage());
    error_log('File: ' . $e->getFile() . ' Line: ' . $e->getLine());
    sendJsonResponse(false, null, 'Error interno del servidor: ' . $e->getMessage(), 500);
} catch (Error $e) {
    error_log('Fatal Error en save-default-template.php: ' . $e->getMessage());
    error_log('File: ' . $e->getFile() . ' Line: ' . $e->getLine());
