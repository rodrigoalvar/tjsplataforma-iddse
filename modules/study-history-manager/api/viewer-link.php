<?php
/**
 * Genera URL para abrir un estudio del historial (Orthanc local o remoto según visor del usuario).
 */
...
require_once __DIR__ . '/../includes/OrthancUidLookup.php';
require_once __DIR__ . '/../includes/RemoteViewerUrlBuilder.php';
...
try {
    $user = requireStudyHistoryAuth('study_history_manager');
    ...
    if ($uid === '') {
        studyHistoryJsonError('study_instance_uid es requerido', 400);
    }
    ...
    if ($vr && !empty(trim((string) $vr['dicom_viewer']))) {
        $viewerType = trim($vr['dicom_viewer']);
    }
    ...
    // Solo abrir en Orthanc local si el usuario no eligió explícitamente un nodo remoto en el historial.
    ...
    if (!$node) {
        studyHistoryJsonError('Nodo no encontrado o inactivo', 404);
    }
    ...
    if ($viewerType === 'UDV' && $remoteMode === 'wado_manifest') {
        ...
        if (($node['node_type'] === 'dimse' || $node['node_type'] === 'hybrid')
            ...
            try {
                ...
                error_log('[STUDY_HISTORY] viewer-link sync: ' . $e->getMessage());
            }
        }
    ...
    ob_end_clean();
    error_log('[STUDY_HISTORY] viewer-link: ' . $e->getMessage());
