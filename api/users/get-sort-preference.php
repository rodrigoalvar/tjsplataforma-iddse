<?php
/**
 * API para obtener la preferencia de ordenamiento del usuario
 */
...
try {
    ...
    $stmt = $db->prepare("
        ...
        WHERE user_id = :user_id AND preference_key = 'dashboard_sort'
    ");
    ...
    if ($result && $result['preference_value']) {
        ...
        if ($preference && isset($preference['column']) && isset($preference['direction'])) {
            echo json_encode([
                'success' => true,
                'data' => [
    ...
    error_log("Error en users/get-sort-preference.php: " . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false,
